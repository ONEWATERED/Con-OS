...
const navStructure = [
  ...
  },
  {
    ...
    color: 'purple', // A neutral color for communications
    tools: [
...
const App: React.FC = () => {
  ...
  const getNavButtonClass = (toolId: AppTool, phaseColor: string) => {
    ...
    const borderColor = phaseColor === 'purple' ? 'border-purple-500' : `border-${phaseColor}`;
    const hoverBorderColor = phaseColor === 'purple' ? 'hover:border-purple-500/50' : `hover:border-${phaseColor}/50`;

    if (selectedProject?.activeTool === toolId) {
      return `${baseClass} ${colorClass} ${borderColor} bg-card shadow-sm text-text-default`;
    }
    return `${baseClass} ${colorClass} border-transparent text-text-muted hover:bg-card/60 ${hoverBorderColor}`;
  };
  ...
  const renderContent = () => {
    ...
    if (view === 'project' && selectedProject) {
      ...
      return (
          <div className="grid grid-cols-1 md:grid-cols-[220px_1fr] lg:grid-cols-[260px_1fr] gap-8">
            <aside>
                <div className="bg-card p-4 rounded-xl shadow-sm border border-border">
                    <h2 className="text-lg font-bold text-text-default truncate">{selectedProject.name}</h2>
                    <p className="text-sm text-text-muted">{selectedProject.address}</p>
                    <button onClick={() => { setView('dashboard'); setSelectedProjectId(null); }} className="mt-4 text-sm text-primary-dark font-semibold hover:underline">
                        &larr; Back to Dashboard
                    </button>
                    <div className="mt-4 pt-4 border-t border-border space-y-2">
                        <button 
                            onClick={() => setIsClientView(true)}
                            className="w-full inline-flex items-center justify-center px-4 py-2 text-sm font-semibold bg-indigo-600 text-white rounded-lg shadow-sm hover:bg-indigo-700"
                        >
                         ...
                         <button 
                            ...
                            className="w-full inline-flex items-center justify-center px-4 py-2 text-sm font-semibold bg-gray-600 text-white rounded-lg shadow-sm hover:bg-gray-700"
                        >
            ...
            <main>
                ...
                {activeTool === 'changeOrder' && <ChangeOrderGenerator project={selectedProject} />}
      ...
      return (
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
              <div className="bg-card rounded-xl shadow-2xl p-8 w-full max-w-md relative">
                  <button onClick={onClose} className="absolute top-4 right-4 text-text-muted hover:text-text-default">&times;</button>
                  <h2 className="text-2xl font-bold mb-6 text-text-default">Create New Project</h2>
                  <form onSubmit={handleSubmit} className="space-y-4">
                      <div>
                          <label className="block text-sm font-medium text-text-muted mb-1">Project Name</label>
                          <input type="text" name="name" onChange={handleChange} value={data.name} className="w-full px-3 py-2 bg-white border border-border rounded-lg shadow-sm" required />
                      </div>
                       <div>
                          <label className="block text-sm font-medium text-text-muted mb-1">Project Address</label>
                          <input type="text" name="address" onChange={handleChange} value={data.address} className="w-full px-3 py-2 bg-white border border-border rounded-lg shadow-sm" required />
                      </div>
                       <div>
                          <label className="block text-sm font-medium text-text-muted mb-1">Client Name</label>
                          <input type="text" name="clientName" onChange={handleChange} value={data.clientName} className="w-full px-3 py-2 bg-white border border-border rounded-lg shadow-sm" required />
                      </div>
                      <button type="submit" className="w-full mt-4 px-6 py-3 bg-primary text-black font-semibold rounded-lg shadow-sm hover:bg-primary-dark">Create Project</button>
                  </form>
              </div>
          </div>
      );
  ...
  return (
    <div className="min-h-screen bg-background font-sans">
      <header className="bg-card border-b border-border shadow-sm sticky top-0 z-10 bg-opacity-80 backdrop-blur-md">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
               <div className="p-2 bg-primary-light rounded-lg">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-primary-dark" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                  </svg>
               </div>
              <div>
                <h1 className="text-2xl font-bold text-text-default">AI Construction Suite</h1>
                <p className="text-text-muted mt-1 text-sm">A lifecycle toolkit for modern contractors.</p>
              </div>
            </div>
            <div className="flex items-center space-x-2">
              ...
              <button onClick={() => setView('crm')} className="p-2 rounded-full hover:bg-gray-100" aria-label="CRM">
              ...
              <button onClick={() => setView('settings')} className="p-2 rounded-full hover:bg-gray-100" aria-label="Company Settings">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-text-muted" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
