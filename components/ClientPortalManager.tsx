                ...
                <div className="flex-grow grid grid-cols-1 md:grid-cols-[1fr_300px] overflow-hidden">
                    <div className="p-4 space-y-4 overflow-y-auto">
                        ...
                        <div>
                            ...
                            <input type="text" value={draft.title} onChange={e => setDraft(d => ({...d, title: e.target.value}))} className="w-full mt-1 p-2 border border-border rounded-md" />
                        </div>
                        <div>
                            <label className="text-sm font-medium text-text-muted">Summary</label>
                            <input type="text" value={draft.summary} onChange={e => setDraft(d => ({...d, summary: e.target.value}))} className="w-full mt-1 p-2 border border-border rounded-md" />
                        </div>
                        
                        <div className="space-y-3">
                            {draft.sections.map(section => (
                                <div key={section.id} className="p-3 border border-border rounded-lg">
                                    <input type="text" value={section.heading} onChange={e => handleSectionChange(section.id, 'heading', e.target.value)} className="w-full p-1 font-semibold text-lg border-b-2 border-transparent focus:border-primary outline-none" />
                                    <textarea value={section.content} onChange={e => handleSectionChange(section.id, 'content', e.target.value)} rows={5} className="w-full mt-2 p-1 border-none focus:ring-1 focus:ring-primary rounded-sm" />
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-6">
            ...
            {editingUpdate && (
                <UpdateEditorModal
                    project={project}
                    update={project.clientUpdates.find(u => u.id === editingUpdate.id) || null}
