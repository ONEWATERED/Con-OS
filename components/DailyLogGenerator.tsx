            ...
            <div className="bg-card rounded-xl shadow-2xl p-8 w-full max-w-lg relative">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="notes" value={formData.notes} onChange={handleChange} rows={10} className={inputClass} placeholder="Enter crew, work completed, delays, safety notes..." required />
                    </div>
...
export const DailyLogManager: React.FC<DailyLogManagerProps> = ({ project, onUpdateProject }) => {
    ...
    const handleCreateRevision = (log: DailyLog) => {
        setEditingLog({
            date: new Date().toISOString().split('T')[0],
            notes: `REVISION of Log from ${log.date}:\n----------------------------------\n${log.notes}`,
            status: 'Draft',
            revisionOf: log.id,
        });
    };
                ...
                <div className="space-y-4">
                    {sortedLogs.map(log => (
                        <div key={log.id} className="bg-card p-4 rounded-xl shadow-sm border border-border">
                            ...
                            <pre className="mt-4 text-sm whitespace-pre-wrap font-sans bg-gray-50 p-3 rounded-md">{log.notes}</pre>
                        </div>
                    ))}
                </div>
            )}
