...
export const EmailInbox: React.FC<EmailInboxProps> = ({ project, companySettings, onUpdateEmails }) => {
  ...
  const FolderButton: React.FC<{
    ...
    <button onClick={onClick} className={`flex items-center justify-between w-full px-4 py-2 text-sm rounded-r-full transition-colors ${isActive ? 'bg-primary-light text-primary-dark font-semibold' : 'text-text-muted hover:bg-gray-100'}`}>
  ...
  return (
    <div className="bg-card rounded-xl shadow-sm border border-border h-[calc(100vh-12rem)] flex overflow-hidden">
        ...
        {/* Email List */}
        <div className="w-[350px] border-r border-border flex flex-col overflow-y-auto bg-gray-50/50">
            {emailsToList.length === 0 ? (
                 <div className="flex-grow flex items-center justify-center text-center text-text-muted p-4">
                        ...
                        <li key={email.id}>
                            ...
                            >
                                <div className="flex justify-between items-start">
                                    <p className={`text-sm truncate ${email.read ? 'font-medium text-text-muted' : 'font-bold text-text-default'}`}>{activeFolder === 'inbox' ? email.from : `To: ${email.to}`}</p>
                                    <p className="text-xs text-text-muted flex-shrink-0 ml-2">{new Date(email.timestamp).toLocaleDateString()}</p>
                                </div>
                                <p className={`mt-1 text-sm truncate ${email.read ? 'text-text-muted' : 'text-text-default'}`}>{email.subject}</p>
                            </button>
                        </li>
                    ))}
                </ul>
