        ...
        <div className={`bg-card p-5 rounded-xl shadow-sm border ${inspection.isSigned ? 'border-gray-300 bg-gray-50' : 'border-border'}`}>
            ...
            {isFailing ? (
                <div className="mt-4 p-4 bg-red-50 border border-red-200 rounded-lg space-y-2">
                    ...
                    <textarea value={failureNotes} onChange={e => setFailureNotes(e.target.value)} rows={3} className="w-full p-2 border border-red-300 rounded-md" placeholder="Enter detailed reason..." required />
            ...
            {isFinalState && !inspection.isSigned && (
                <div className="mt-4 pt-4 border-t border-border space-y-2 bg-yellow-50/50 p-4 rounded-lg">
                    ...
                    <div className="flex items-center space-x-2">
                        <input type="text" value={signerName} onChange={e => setSignerName(e.target.value)} className="w-full px-3 py-1.5 text-sm border border-border rounded-md" placeholder="Type signer's name..."/>
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      {sortedInspections.length === 0 ? (
        <div className="text-center py-16 px-6 bg-card rounded-xl shadow-sm border border-border">
          <svg xmlns="http://www.w3.org/2000/svg" className="mx-auto h-12 w-12 text-text-muted" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
          <h2 className="mt-4 text-xl font-semibold text-text-default">No Inspections Requested</h2>
          <p className="mt-2 text-text-muted">Click the button to create and track your first inspection request.</p>
        </div>
      ) : (
