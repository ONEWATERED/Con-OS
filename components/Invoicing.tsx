                ...
                <div className="flex-grow overflow-y-auto border-y border-border -mx-6 px-6 py-2">
                    {expenses.length === 0 ? (
                        ...
                        <ul className="divide-y divide-border">
                            {expenses.map(exp => (
                                <li key={exp.id} className="flex items-center space-x-3 p-3 cursor-pointer hover:bg-gray-50" onClick={() => handleToggle(exp.id)}>
                                    <input
                                        type="checkbox"
                    ...
                    {Object.keys(groupedByEmployee).length === 0 ? (
                        ...
                        <ul className="divide-y divide-border">
                            {Object.entries(groupedByEmployee).map(([employeeId, data]) => (
                                <li key={employeeId} className="p-3 cursor-pointer hover:bg-gray-50" onClick={() => handleToggle(employeeId)}>
...
export const Invoicing: React.FC<InvoicingProps> = ({ project, companySettings, onUpdateProject, allContacts }) => {
  ...
  const inputClass = "w-full px-2 py-1 bg-white border border-border rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary text-sm";
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="bg-card rounded-xl shadow-sm border border-border overflow-hidden">
        ...
        {!project.usesScheduleOfValues && (
          <div className="p-4 border-t border-border">
            <button onClick={handleAddLineItem} className="px-3 py-1.5 text-sm font-medium bg-primary-light/80 hover:bg-primary-light rounded-md">Add Line Item</button>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-card p-4 rounded-xl shadow-sm border border-border">
          ...
          <button onClick={handleAddChangeOrder} className="mt-3 px-3 py-1.5 text-sm font-medium bg-primary-light/80 hover:bg-primary-light rounded-md">Add Change Order</button>
        </div>
      </div>
       {isExpenseModalOpen && (
            <AddExpensesModal 
