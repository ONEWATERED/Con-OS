...
const EstimateView: React.FC<{ 
  ...
  projectUsesSov: boolean,
    ...
    const inputClass = "w-full p-1 bg-transparent border-none focus:ring-1 focus:ring-primary focus:bg-white rounded-sm";
                ...
                <div className="flex justify-between items-center">
                    <h3 className="text-lg font-semibold text-text-default">Line Items (Editable)</h3>
                     <button 
                        ...
                        className="px-4 py-2 text-sm font-semibold bg-green-600 text-white rounded-lg shadow-sm hover:bg-green-700 disabled:bg-gray-400 disabled:cursor-not-allowed"
                    ...
                    <table className="min-w-full divide-y divide-border">
                        ...
                        <tbody className="bg-white divide-y divide-border">
                            {lineItems.length > 0 ? lineItems.map((item, index) => (
                                <tr key={index} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-4 py-3 text-sm text-text-default font-semibold text-right">{formatCurrency(item.lineTotal || 0)}</td>
                                </tr>
                            )) : (
                        ...
                        <tfoot>
                            <tr className="bg-gray-50/80">
                                <td colSpan={4} className="px-4 py-3 text-right text-sm font-bold text-text-default uppercase">Total Estimated Cost</td>
...
const TextView: React.FC<{ title: string; text: string; companySettings: CompanySettings }> = ({ title, text, companySettings }) => (
    <div>
        ...
        <div className="relative p-4 bg-gray-50 border border-border rounded-lg">
            ...
            <pre className="whitespace-pre-wrap font-sans text-sm text-text-default overflow-x-auto">{text || 'No content generated.'}</pre>
