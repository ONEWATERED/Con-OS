...
const RfiCard: React.FC<{
  ...
  const handleAnswerChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    onUpdate({ ...rfi, answer: e.target.value });
  };
    ...
    <div className="bg-card p-5 rounded-xl shadow-sm border border-border space-y-4">
      ...
      <div>
        ...
        <textarea
          ...
          className="w-full px-3 py-2 bg-white border border-border rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-primary/50"
        ...
        <div className="border-t border-border pt-4 space-y-3">
          ...
          {rfi.analysis && (
            <div className="p-3 bg-gray-50 border border-border rounded-lg">
                <pre className="whitespace-pre-wrap font-sans text-sm text-text-default">{rfi.analysis}</pre>
            </div>
          )}
        </div>
      )}
      ...
      <div className="border-t border-border pt-4 space-y-3">
        ...
        <div className="space-y-2 max-h-40 overflow-y-auto pr-2 -mr-2 simple-scrollbar">
            ...
            {rfi.log && rfi.log.map((entry, index) => (
                <div key={index} className="text-xs p-2 bg-gray-50 rounded-md border border-gray-200">
                    ...
                    <p className="text-text-muted mt-1">{entry.note}</p>
                </div>
            ))}
        </div>
        ...
        <form onSubmit={handleAddNote} className="flex items-start space-x-2 pt-2">
            <textarea
                ...
                rows={2}
                className="flex-grow px-3 py-2 text-sm bg-white border border-border rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-primary/50"
  ...
  if (rfis.length === 0) {
    return (
      <div className="text-center py-16 px-6 bg-card rounded-xl shadow-sm border border-border">
        <svg xmlns="http://www.w3.org/2000/svg" className="mx-auto h-12 w-12 text-text-muted" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 8l-4-4m0 0l4-4m-4 4h12" />
        </svg>
        <h2 className="mt-4 text-xl font-semibold text-text-default">RFI Manager is Empty</h2>
        <p className="mt-2 text-text-muted">
          Generate RFIs using the 'Estimator' tool, then click 'Send to RFI Manager' to start tracking them here.
        </p>
      </div>
    );
  }
