            ...
            <div className="bg-card rounded-xl shadow-2xl p-6 w-full max-w-md relative">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        ...
                        <input type="number" value={hours} onChange={e => setHours(parseFloat(e.target.value) || 0)} className="w-full mt-1 p-2 border rounded-md" step="0.25" min="0" required />
                    </div>
                    <div>
                        <label className="block text-sm font-medium">Cost Code</label>
                        <select value={costCode} onChange={e => setCostCode(e.target.value)} className="w-full mt-1 p-2 border rounded-md bg-white">
                    ...
                    <div className="p-3 rounded-md bg-gray-50 border">
                        <h3 className="text-sm font-semibold mb-2">Location Verification</h3>
                    ...
                    <div className="flex justify-end space-x-3 pt-4">
                        ...
                        <button type="submit" disabled={isSaveDisabled} className="px-6 py-2 bg-primary font-semibold rounded-lg shadow-sm disabled:bg-gray-400 disabled:cursor-not-allowed">Save</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    return (
        <div className="bg-card p-6 rounded-xl shadow-sm border border-border">
            <div className="flex justify-between items-center mb-4">
                ...
                <h3 className="font-semibold">{weekStart} to {weekEnd}</h3>
                ...
                {dailyData.map(({ date, dateStr, totalHours, status, location }) => (
                    <div key={dateStr}>
                        ...
                        <div 
                            ...
                            className={`relative mt-2 p-4 border rounded-lg h-24 flex flex-col justify-between ${isManagerView ? '' : 'cursor-pointer hover:bg-gray-100'} ${status ? statusColors[status] : 'border-dashed'}`}
                        >
            ...
            <div className="mt-4 pt-4 border-t flex justify-between items-center">
                ...
                {!isManagerView && hasDrafts && (
                    <button onClick={() => handleWeekSubmit('submit')} className="px-4 py-2 bg-green-600 text-white font-semibold rounded-lg shadow-sm">Submit Week for Approval</button>
                )}
                 {isManagerView && weekEntries.some(e => e.status === 'Pending') && (
                    <div className="space-x-2">
                         <button onClick={() => handleWeekSubmit('reject')} className="px-4 py-2 bg-red-600 text-white font-semibold rounded-lg shadow-sm">Reject Week</button>
                         <button onClick={() => handleWeekSubmit('approve')} className="px-4 py-2 bg-green-600 text-white font-semibold rounded-lg shadow-sm">Approve Week</button>
                    </div>
                 )}
            </div>
                    ...
                    {teamSummary.map(member => (
                        <li key={member.id}>
                            <button 
                                ...
                                className={`w-full text-left p-3 rounded-lg ${selectedEmployeeId === member.id ? 'bg-primary-light' : 'hover:bg-gray-100'}`}
                            >
